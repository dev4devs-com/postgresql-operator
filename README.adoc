...
TIP: If you get an `oc: command not found` just run `eval $(minishift oc-env)` and you'll be good to give it another try
...
```shell
$ oc describe Database | grep "Database Status"
...
  Database Status:  OK
...
```

TIP: If you get no output from the previous command, you may need to jump to the correct project, run `oc project postgresql-operator` before trying again

Now is time to check if all the resources are available as expected, running `kubectl get all -o wide` one should see a result similar to the one below
...
By the specs in link:deploy/crds/postgresql_v1alpha1_database_cr.yaml[Database CR](_deploy/crds/postgresql_v1alpha1_database_cr.yaml_) you are able to customize the setup for this operator. Note that by the spec `configMapName` you are able to inform the name of a configMapName which has the keys and values which the Database should use in its required env vars.
...
. Setup an AWS S3 Bucket in order to store the backup outside of the cluster. You need to add your AWS details to link:./postgresql-operator/deploy/crds/postgresql_v1alpha1_backup_cr.yaml[Backup CR](_postgresql-operator/deploy/crds/postgresql_v1alpha1_backup_cr.yaml_) as following or add the name of the secret which has already this data in the cluster.
+
[source,yaml]
----
  # ---------------------------------
