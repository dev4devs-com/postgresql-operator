...
An https://commons.openshift.org/sig/OpenshiftOperators.html[Operator] based on the https://github.com/operator-framework/operator-sdk[Operator SDK] to package, install, configure and manage an PostgreSQL database. Also, this operator will allow you customize and configure it easily according with your needs.
